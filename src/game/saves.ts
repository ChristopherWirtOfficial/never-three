...
export function migrateRawToSaveState(raw: Record<string, unknown>): SaveState {
	return {
		...
		dice: (Array.isArray(raw.dice) ? (raw.dice as number[][]) : null) ?? [[1, 2, 3, 4, 5, 6]],
		totalDieReforgeCount: numOpt(raw, 0, 'totalDieReforgeCount', 'totalReforges'),
...
export async function persistProfileSnapshot(
	...
	writeLocalStorageString(`nt-save-${id}`, JSON.stringify({ ...state, _v: SAVE_VERSION }))
	if (setActive) {
...
export async function createProfile(
	...
	const name = trimmed && trimmed.length > 0 ? trimmed : formatUnnamedSaveLabel()
	const id = makeId()
